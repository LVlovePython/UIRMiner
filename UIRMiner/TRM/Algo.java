...
public class Algo {
    ...
    final boolean complementRSU = true; // if UIRMiner use complement utility pruning strategy.
...
//        writer = new BufferedWriter(new FileWriter(output));
        ...
        System.out.println(" maxRelation: " + maxRelation + " max int: " + Integer.MAX_VALUE + " " + (maxRelation < Integer.MAX_VALUE));
        // write the info
        String buffer = "=============TRule Miner STATS=============\n" +
                " dataset: " + dataset.substring(73) + '\n' +   // this number should be changed as the input filename.
    ...
    public void save(ArrayList<Integer> antecedent, ArrayList<Integer> consequent, ArrayList<Integer> anteRelation,
                     ArrayList<Integer> conRelation, int utility, int ruleSup, double ruleConfidence) throws IOException {
        ...
        for (Integer ent: allConseEvtb) {
            for (int r = 0; r < 7; r++) {
                ...
                if(mapEntbRSU[2] >= threshold && (double)mapEntbRSU[1] / projectedAnteDB.size() >= confidence) {
                    ...
                    consequent.add(ent);
                    ArrayList<Integer> conRelation = new ArrayList<>();
                    conRelation.add(r);
                    ...
                    if (printOutput) {
                        System.out.println("rule: " + antecedent.get(0) + "-->" + ent + " with relation: " + r + " utility: " +
                                mapEntbRSU[2] + " confidence: " + (double)mapEntbRSU[1] / projectedAnteDB.size() + " ante sup: " + projectedAnteDB.size() +
                                " con sup: " + mapEntbRSU[1]);
                    }
                }
                ...
                if (antecedent.size() < maxAntecedent) {
                    if(mapEntbRSU[0] >= threshold) {
                        ...
                        for (int evt: setEvtExtendAnte) {
                            ...
                            ArrayList<Integer> anteRelation = new ArrayList<>();
                            ArrayList<Integer> conseRelation = new ArrayList<>();
                            conseRelation.add(r);
                ...
                if(mapEntbRSU[3] >= threshold && (double)mapEntbRSU[1] / projectedAnteDB.size() >= confidence) {
                    ...
                    for (Integer evt: setEvtExtendConse) {
                        ...
                        if (RSU < threshold) continue;
                        ArrayList<ProjectSequence> newProjectedDB = lastEvt.projectDB;
                        ArrayList<Integer> globalRelation = new ArrayList<>();
                        globalRelation.add(r);
    ...
    private int extendConse(ArrayList<ProjectSequence> projectedDB, int evt, ArrayList<Integer> antecedent, ArrayList<Integer> anteRelation
            , ArrayList<Integer> consequent, ArrayList<Integer> globalRelation, int relation, double anteSup, int maxConsequent) throws IOException {
        candidate++;
        int rsu = 0;
        ...
        if (rsu >= threshold) {
            HashMap<Integer, Integer> relRSU = new HashMap<>(); // <ruleRelation: rsu>
            HashMap<Integer, Integer> relUtility = new HashMap<>();   // <ruleRelation: utility>
            HashMap<Integer, Integer> relSupport = new HashMap<>();   // <ruleRelation: support>
            HashMap<Integer, ArrayList<ProjectSequence>> relProDB = new HashMap<>();   // <ruleRelation: DB>
            ...
            for (ProjectSequence projectSequence : projectedDB) {
                ...
                if (relation != getRelation(elements.get(eventPos), elements.get(idxs.get(consequent.get(consequent.size() - 1)))))
                    continue;
                int last;
                if (globalRelation == null) last = 0;
                else last = globalRelation.get(globalRelation.size() - 1);
                int rel = getAllRelations(elements, idxs, antecedent, consequent, last, evt);
//            System.out.println(rel);
            ...
            consequent.add(evt);
            for (Map.Entry<Integer, Integer> entry: relRSU.entrySet()) {
                int newRelation = entry.getKey();
                if (entry.getValue() >= threshold && (double)relSupport.get(newRelation) / anteSup >= confidence) {
                    ...
                    if (newDB == null) System.out.println("ERROR");
    ...
    private void runExtendConse(ArrayList<ProjectSequence> projectDB, ArrayList<Integer> antecedent, ArrayList<Integer> anteRelation
            , ArrayList<Integer> consequent, ArrayList<Integer> ruleRelation, double anteSup, int maxConsequent) throws IOException {
    ...
    private LastEvt getEvtProjectDBC(int evt, ArrayList<ProjectSequence> projectedAnteDB, ArrayList<Integer> antecedent, ArrayList<Integer> consequent, int relation) {
        int RSU = 0;
        ...
        for (ProjectSequence projectSequence : projectedAnteDB) {
            ...
            int r = getAllRelations(elements, idxs, rule, consequent.get(consequent.size() - 1));
    ...
    private HashSet<Integer> getSetEvtExtendConse(ArrayList<ProjectSequence> projectedDB, ArrayList<Integer> antecedent, ArrayList<Integer> consequent, int relation) {
    ...
    private int extendAnte(ArrayList<ProjectSequence> projectedDB, int event, ArrayList<Integer> antecedent, ArrayList<Integer> anteRelation,
                           int ent, ArrayList<Integer> globalRelation, int r, int maxAntecedent, int maxConsequent) throws IOException {
        candidate++;
        int rsu = 0;
        int roughRSU = 0;
        for(ProjectSequence projectSequence: projectedDB) {
        ...
        if (roughRSU >= threshold) {
            HashMap<Integer, HashMap<Integer, Integer>> relRSU = new HashMap<>(); // <anteRel: <conseRel: rsu>>
            HashMap<Integer, HashMap<Integer, Integer>> relUtility = new HashMap<>();   // <anteRel: <conseRel: utility>>
            HashMap<Integer, HashMap<Integer, Integer>> relSupport = new HashMap<>();   // <anteRel: <conseRel: support>>
            HashMap<Integer, ArrayList<ProjectSequence>> relProDB = new HashMap<>();   // <anteRel: DB>
            for (ProjectSequence projectSequence : projectedDB) {
                VHRepresentation seq = projectSequence.getSequence();
                ArrayList<VHElement> elements = seq.horizontalList;
                HashMap<Integer, Integer> idxs = seq.verticalMap;
                ...
                int rel = getAllRelations(elements, idxs, antecedent, last, event);
                ArrayList<ProjectSequence> newDB = relProDB.get(rel);
                EPosition ePosition = new EPosition(eventPos, projectSequence.getEPositions().getUtility()
                ...
                if (idxs.get(ent) != null) { //if(idxs.get(ent) != null) // a legal candidate rule
                    ...
                    if (eventPos < samPos) { // for a legal rule, update its utility and ub
                        ...
                        if (getAllRelations(elements, idxs, antecedent, ent) != globalRelation.get(globalRelation.size() - 1))
                            continue;
                        int newGlobalRelation = globalRelation.get(globalRelation.size() - 1) * 7 + getRelation(elements.get(idxs.get(ent)), elements.get(idxs.get(event)));
                        ...
                        HashMap<Integer, Integer> rRSU = relRSU.get(rel);
                        HashMap<Integer, Integer> rSupport = relSupport.get(rel);
                        HashMap<Integer, Integer> rUtility = relUtility.get(rel);
            ...
            conse.add(ent);
            int g = globalRelation.get(globalRelation.size() - 1);
            for (Map.Entry<Integer, HashMap<Integer, Integer>> entry: relRSU.entrySet()) {
                int relation = entry.getKey();
                for (Map.Entry<Integer, Integer> specificEntry: entry.getValue().entrySet()) {
                    int gRelation = specificEntry.getKey();
//                if (antecedent.size() == 3) System.out.println("RSU: " + specificEntry.getValue());
                        ...
                        if (relUtility.get(relation).get(gRelation) >= threshold && (double)relSupport.get(relation).get(gRelation)
                            ...
                            if (printOutput) {
                                System.out.println("rule: " + antecedent + "-->" + ent + " with relation: " + anteRelation
                                        ...
                                        + (double)relSupport.get(relation).get(gRelation) / relProDB.get(relation).size() + " ante sup: " + relProDB.get(relation).size() +
                                        " con sup: " + relSupport.get(relation).get(gRelation));
                            }
                        }
                        if (antecedent.size() < maxAntecedent) {
                            MemoryLogger.getInstance().checkMemory();
    ...
    private void runExtendAnte(ArrayList<ProjectSequence> projectSequencesDB, ArrayList<Integer> antecedent, ArrayList<Integer> anteRelation,
                               int consequent, ArrayList<Integer> globalRelation, int maxAntecedent, int maxConsequent) throws IOException {
...
//             extend as the order of b, m, o, e, f, c, s
    ...
    private int getAllRelations(ArrayList<VHElement> elements, HashMap<Integer, Integer> idxs, ArrayList<Integer> antecedent, int evt) {
        int relA = 0;
        ...
        maxRelation = Math.max(maxRelation, relA);
        return relA;
    }

    private int getAllRelations(ArrayList<VHElement> elements, HashMap<Integer, Integer> idxs, ArrayList<Integer> antecedent,
                                ArrayList<Integer> consequent, int rRelation, int evt) {
        int relA;
    ...
    private int getAllRelations(ArrayList<VHElement> elements, HashMap<Integer, Integer> idxs, ArrayList<Integer> antecedent, int anteRelation, int evt) {
        int relA;
    ...
    private int[] getMapEvtbRSU(ArrayList<ProjectSequence> projectedAnteDB, int ante, int conse, int r) {
        ...
        for (ProjectSequence sequence: projectedAnteDB) {
            ...
            int localRSUAnte = antecedent.utility + consequent.utility + consequent.ru + antecedent.ru - elements.get(idxs.get(conse)).ru
                    - elements.get(idxs.get(conse)).utility;
